...
# 或
pnpm dev              # 开发：后端 3000 + 前端 5173 双热更
pnpm watch            # 自研 fs.watch，保存即重编重启
pnpm demo             # 另起终端跑 mock 上游 4000，零成本试
```

日常 `pnpm start` 就够。一条命令，别多开。`.env` 指到 mock 时先 `pnpm demo` 再 `pnpm start`。改前端记得 `pnpm build`。
