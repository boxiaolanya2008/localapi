...
- 使用记录新增时间范围统计:今日/近7天/近30天/自定义xxx-xxx/全部,顶部统计卡实时显示该范围(叠加密钥/渠道筛选)的请求数/Token/缓存命中/费用
- 表头悬浮提示同步(7 日图大数压缩)

## 0.2.9 - 2026-08-18

- 参数模板页新增「应用到分组」:直接选分组把该模板绑定过去,并显示当前使用该模板的分组
- 仪表盘 7 日 Token 图:大数值压缩显示(如 2,700,379 → 2.70M),修复长数字把坐标轴/悬浮提示挤爆

## 0.2.8 - 2026-08-18

- 修复无 usage 时 token 统计被 JSON 结构开销夸大的问题:估算只看真实消息/回复文本(非流式取 message 正文,流式只累加 delta.content),今日/累计 token 与费用恢复合理
